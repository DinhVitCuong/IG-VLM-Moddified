...
from model_processor.llava2_model_processor import Llava157BProcessor, LlavaNeXTVideo7BProcessor, Llava16Vicuna7BProcessor, Llava_Onevision_QWEN2_05BProcessor
from vision_processor.fps_gridview_processor import FpsDataProcessor, FpsExtractor
...
class Llava16_Vicuna_7B_Pipeline:
    def __init__(
        self,
        model_name,
        path_qa,
    ...
    def load_model(self):
        print("[DEBUG] Initializing Llava16_Vicuna_7BProcessor")
        self.model = Llava16Vicuna7BProcessor(self.model_name)
        self.model.load_model()
        print("[DEBUG] Model loaded in LlavaPipeline")
...
class Llava_NeXT_Video_7B_Pipeline:
    def __init__(
        self,
        model_name,
        path_qa,
    ...
    def load_model(self):
        print("[DEBUG] Initializing LlavaNeXTVideo7BProcessor")
        self.model = LlavaNeXTVideo7BProcessor(self.model_name)
        self.model.load_model()
        print("[DEBUG] Model loaded in LlavaPipeline")
    ...
    def set_component(
        ...
        self.frame_fixed_number = frame_fixed_number
        self.calculate_max_row = calculate_max_row
        self.fps_extractor = FpsExtractor(video_path)
        

        extra_dir = f"ffnvideo={self.frame_fixed_number}/"
        self._make_directory(extra_dir)
    ...
    def do_pipeline(self, qa_text=None):
        ...
        try:
            video_data = self.fps_extractor.save_data_based_on_option(
                "numpy",
                frame_fixed_number=self.frame_fixed_number,
                ts=ts,
            )
            # print("[DEBUG]: VIDEO PROCESSED")
            if video_data == -1:
                print(f"Failed to process video: {video_path}")
                self.error_video_name.append(video_path)
                return None, None, None
            ...
            user_prompt = self.processor.apply_chat_template(conversation, add_generation_prompt=True)
            answer = self.model.inference(
...
class Llava_Onevision_QWEN2_05BPipeline:
    def __init__(
        self,
        model_name,
        path_qa,
    ...
    def load_model(self):
        print("[DEBUG] Initializing Llava-OneVision-QWEN-0.5BProcessor")
        self.model = Llava_Onevision_QWEN2_05BProcessor(self.model_name)
        self.model.load_model()
        print("[DEBUG] Model loaded in LlavaPipeline")
